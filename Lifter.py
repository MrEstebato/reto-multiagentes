...
class Lifter:
    ...
    def update(self, delta):
        ...
        mssg = (
            "Agent:%d \t State:%s \t Position:[%0.2f,0,%0.2f] \t NodoActual:%d \t NodoSiguiente:%d"
            % (
                self.idx,
                self.status,
                self.Position[0],
        ...
        match self.status:
            ...
            case "lifting":
                if self.platformHeight >= 0:
                    ...
                    self.platformHeight += delta
            case "delivering":
                if (self.Position[0] <= 10 and self.Position[0] >= -10) and (
            ...
            case "dropping":
                if self.platformHeight <= -1.5:
                    ...
                    self.platformHeight -= delta
            case "returning":
                if (self.Position[0] <= 20 and self.Position[0] >= -20) and (
                    self.Position[2] <= 20 and self.Position[2] >= -20
                ):
