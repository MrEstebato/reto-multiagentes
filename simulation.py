...
def Init(Options):
    ...
    lifters.append(
        Lifter(Settings.DimBoardX, 0.7, textures, 0, Positions[0], CurrentNode)
    )
...
def drawLShapeWall():
    ...
    # Draw first segment
    glTranslatef(
        -Settings.DimBoardX + width1 / 2, 0.0, -25
    )  # Position for first segment
    ...
    # Draw second segment
    glTranslatef(
        width1 / 2 + width2 / 2, 0.0, depth1 - 102
    )  # Position for second segment
...
def drawRefri():
    ...
    glPushMatrix()
    glTranslatef(
        -Settings.DimBoardX + width / 2, 0, Settings.DimBoardZ - depth / 2
    )  # Center of the plane
...
def drawFreezer():
    ...
    glPushMatrix()
    glTranslatef(
        -Settings.DimBoardX + width * 2.5, 0, Settings.DimBoardZ - depth / 2
    )  # Center of the plane
